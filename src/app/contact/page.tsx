          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: '5rem', alignItems: 'start', maxWidth: '1000px', margin: '0 auto' }} className="contact-grid">
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                {[
                  { emoji: '📞', label: 'Phone', value: '(305) 600-5727' },
                  { emoji: '✉️', label: 'Email', value: 'hello@bizgenies.com' },
                  { emoji: '📍', label: 'Location', value: 'Miami, FL (Serving Nationwide)' },
                ].map(item => (
                  <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    <div style={{ width: '3rem', height: '3rem', borderRadius: '10px', background: 'rgba(255,107,53,.1)', border: '1px solid rgba(255,107,53,.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.25rem', flexShrink: 0 }}>{item.emoji}</div>
                    <div>
            ...
            {/* Right — form */}
            <div style={{ background: 'var(--surface)', borderTop: '4px solid var(--accent)', borderRadius: '12px', padding: '2.5rem', boxShadow: '0 24px 64px rgba(0,0,0,.35)' }}>
              {submitted ? (
                ...
                <>
                  ...
                  <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                    ...
                    <select className="input" required value={form.businessType} onChange={e => setForm({ ...form, businessType: e.target.value })} style={{ color: form.businessType ? 'var(--text)' : 'var(--muted)', cursor: 'pointer' }}>
                    ...
                    <button type="submit" disabled={loading} className="btn btn-primary" style={{ width: '100%', fontSize: '1.05rem', padding: '1rem' }}>
                      {loading ? (
                        <><span className="animate-spin" style={{ display: 'inline-block', width: '1rem', height: '1rem', border: '2px solid #fff', borderTopColor: 'transparent', borderRadius: '50%', marginRight: '.5rem' }} />Sending...</>
                      ) : 'Book My Free Call →'}
                    </button>
